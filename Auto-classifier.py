...
# Create inputs for the different features of the data
    if add_selectbox == 'Online':
        ...
        shield_jack_stroke_right = st.number_input('Shield jack stroke right (mm)', min_value=float(data['shield_jack_stroke_right'].min()), value=0)
        propulsion_pressure = st.number_input('Propulsion pressure (MPa)', min_value=float(data['propulsion_pressure'].min()), value=0)
        total_thrust = st.number_input('Total thrust (kN)', min_value=float(data['total_thrust'].min()), value=0)
        cutter_torque = st.number_input('Cutter torque (kNm)', min_value=float(data['cutter_torque'].min()), value=0)
        cutterhead_rotation_speed = st.number_input('Cutterhead rotation speed (rpm)', min_value=float(data['cutterhead_rotation_speed'].min()), value=0)
        screw_pressure = st.number_input('Screw pressure (MPa)', min_value=float(data['screw_pressure'].min()), value=0)
        screw_rotation_speed = st.number_input('Screw rotation speed (rpm)', min_value=float(data['screw_rotation_speed'].min()), value=0)
        gate_opening =st.number_input('Gate opening (%)', min_value=float(data['gate_opening'].min()), max_value=100, value=0)
        mud_injection_pressure = st.number_input('Mud injection pressure (MPa)', min_value=float(data['mud_injection_pressure'].min()), value=0)
        add_mud_flow = st.number_input('Add mud flow (L/min)', min_value=float(data['add_mud_flow'].min()), value=0)
        back_in_injection_rate = st.number_input('Back in injection rate (%)', min_value=float(data['back_in_injection_rate'].min()), max_value=100, value=0)
